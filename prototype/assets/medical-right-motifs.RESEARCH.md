...
すぼまる鉢＋右上から差す杵。色は `--gr2`、不透明度 .13
（2026-09-08：ユーザー指示で .11→.13）。`.msb` 帯の右端に半分覗く形
（帯は `overflow:hidden`）。**大きさ：width 100px・right 6px・bottom
-16px**（当初 62px。「右側の空間が余っているぶんをアイコンで示す」
という指示で拡大。130px は大きすぎ→100px に。帯の高さは中身＝
電話／Web／メモで決まるので、シルエットを大きくしても帯は伸びない
――はみ出しは overflow で切る）。
