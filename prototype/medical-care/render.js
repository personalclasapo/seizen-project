...
(function (S) {
  ...
  /* 介護で使うもの｜福祉用具のグリフ。
     ...
     ★素材：**Health Icons（CC0）** を採用した（2026-09-11）。
     出どころ・ライセンス・採否の理由は
     `assets/equip-icons/README.md`。従来の自作グリフ（24×24 の線画）は
     「ベッド＝タブレット」「歩行器＝鳥居」に見えており、CLAUDE.md の
     判定（「それらしさ」に説得力が要るものは PD/CC0 素材を探すのが先）
     に沿って差し替えた。

     ・bed     … healthicons `inpatient`（側面図・ヘッド上がり・キャスター）
     ・other   … healthicons `wheelchair`（車椅子。従来は「箱＋丸」だった）
     ・cane    … healthicons `cane`（杖）
     ・walker  … **CC0 に歩行器が存在しない**ため、Health Icons と同じ
                 規格（viewBox 48×48・塗りのシルエット・2px の骨）で
                 自作した。探索ログは README。

     素材はここに直接埋め込む（`BODY_ART` と同じ理由――file:// で開くと
     fetch が CORS で拒まれる）。元ファイルは `assets/equip-icons/*.svg`
     に残してあるので、差し替えるときはそちらを直してここへ貼り直す。

     全て viewBox 0 0 48 48、パスは `fill="currentColor"` を自前で持つ
     （塗りのシルエット。線画ではないので stroke は使わない）。      */
  const EQUIP_IC = {
    bed:
      '<path fill-rule="evenodd" clip-rule="evenodd" d="M16.0961 11.1052C14.624 9' +
      '.63232 12.2364 9.6315 10.7632 11.1034L10.0591 11.8069C8.71987 11.4263 7.22' +
      '155 11.7669 6.16703 12.8285C4.61099 14.3951 4.61099 16.9376 6.16703 18.504' +
      '2L7 19.3428V30H4V32H6.05051C5.40223 32.6353 5 33.5207 5 34.5C5 36.433 6.56' +
      '7 38 8.5 38C10.433 38 12 36.433 12 34.5C12 33.5207 11.5978 32.6353 10.9495' +
      ' 32H37.0505C36.4022 32.6353 36 33.5207 36 34.5C36 36.433 37.567 38 39.5 38' +
      'C41.433 38 43 36.433 43 34.5C43 33.5207 42.5978 32.6353 41.9495 32H44V30H4' +
      '0V26.8873C41.7218 26.4646 43 24.8993 43 23.033C43 20.8423 41.2388 19.0664 ' +
      '39.0662 19.0664H18.11C18.0285 19.0664 17.9502 19.0337 17.8925 18.9756L17.5' +
      '261 18.6068L17.8945 18.2387C19.3678 16.7667 19.3686 14.3793 17.8963 12.906' +
      '3L16.0961 11.1052ZM16.1167 17.1878L16.4809 16.8239C17.1727 16.1327 17.1731' +
      ' 15.0118 16.4818 14.3202L14.6816 12.5191C13.9902 11.8273 12.8687 11.8269 1' +
      '2.1768 12.5182L11.8263 12.8684L16.1167 17.1878ZM41 23.033C41 21.9311 40.11' +
      '85 21.0664 39.0662 21.0664H18.11C17.4932 21.0664 16.905 20.8194 16.4736 20' +
      '.3851L10.3677 14.238C9.59731 13.4623 8.35643 13.4623 7.586 14.238C6.80467 ' +
      '15.0246 6.80467 16.3081 7.586 17.0948L15.4379 24.9997H39.0662C40.1186 24.9' +
      '997 41 24.1349 41 23.033ZM14.2752 26.6672C14.4867 26.8801 14.7733 26.9997 ' +
      '15.0722 26.9997H38V30H9V21.3563L14.2752 26.6672ZM10 34.5C10 35.3284 9.3284' +
      '3 36 8.5 36C7.67157 36 7 35.3284 7 34.5C7 33.6716 7.67157 33 8.5 33C9.3284' +
      '3 33 10 33.6716 10 34.5ZM39.5 36C40.3284 36 41 35.3284 41 34.5C41 33.6716 ' +
      '40.3284 33 39.5 33C38.6716 33 38 33.6716 38 34.5C38 35.3284 38.6716 36 39.' +
      '5 36Z" fill="currentColor"/>',
    /* 歩行器｜自作（Health Icons 規格に合わせた）。持ち手・枠・
       開いた脚・前脚のキャスター。 */
    walker:
      '<path d="M13 7h22a4 4 0 0 1 4 4v1h-2v-1a2 2 0 0 0-2-2H13a2 2 0 0 0-2 2v1H9' +
      'v-1a4 4 0 0 1 4-4Z"/><path fill-rule="evenodd" d="M8 12h32v2h-1.5l2 24-2 .' +
      '2L36.5 14h-25L9.5 38.2l-2-.2 2-24H8v-2Z"/><path fill-rule="evenodd" d="M17' +
      '.5 14h2v11h9V14h2v11h4v2h-21v-2h4V14Z"/><path fill-rule="evenodd" d="M9 39' +
      'a3.5 3.5 0 1 1 0 7 3.5 3.5 0 0 1 0-7Zm0 2a1.5 1.5 0 1 0 0 3 1.5 1.5 0 0 0 ' +
      '0-3Zm30-2a3.5 3.5 0 1 1 0 7 3.5 3.5 0 0 1 0-7Zm0 2a1.5 1.5 0 1 0 0 3 1.5 1' +
      '.5 0 0 0 0-3Z"/>',
    chair:
      '<path fill-rule="evenodd" clip-rule="evenodd" d="M27.0263 14.5263C29.3808 ' +
      '14.5263 31.2895 12.6176 31.2895 10.2632C31.2895 7.90868 29.3808 6 27.0263 ' +
      '6C24.6718 6 22.7632 7.90868 22.7632 10.2632C22.7632 12.6176 24.6718 14.526' +
      '3 27.0263 14.5263ZM27.0263 12.5263C28.2762 12.5263 29.2895 11.5131 29.2895' +
      ' 10.2632C29.2895 9.01325 28.2762 8 27.0263 8C25.7764 8 24.7632 9.01325 24.' +
      '7632 10.2632C24.7632 11.5131 25.7764 12.5263 27.0263 12.5263Z" fill="curre' +
      'ntColor"/><path fill-rule="evenodd" clip-rule="evenodd" d="M23.5265 13.594' +
      '5C22.35 13.4256 21.3203 13.9496 20.5739 14.6032C19.8234 15.2605 19.243 16.' +
      '1422 18.9153 16.973C18.7387 17.4207 18.5216 18.3523 18.3101 19.4241C18.233' +
      '9 19.8102 18.1561 20.2266 18.0789 20.6634V17.1053L14.8368 14.6737L13.6368 ' +
      '16.2737L16.0789 18.1053V23.679C12.5236 25.044 10 28.4904 10 32.5263C10 37.' +
      '7585 14.2415 42 19.4737 42C24.3705 42 28.3996 38.2847 28.896 33.5193L28.95' +
      '23 33.5152L29.1114 33.7925C29.4017 34.2978 29.801 34.99 30.2369 35.7377C31' +
      '.1029 37.2228 32.1326 38.9595 32.7345 39.8686C33.6679 41.2785 35.3 41.33 3' +
      '6.4039 40.7703C36.964 40.4862 37.4675 40.0173 37.7482 39.3933C38.0387 38.7' +
      '475 38.0621 37.9939 37.7382 37.2472C37.3137 36.2687 36.3116 33.9179 35.413' +
      '7 31.8075C34.9652 30.7532 34.5433 29.7603 34.2335 29.031L34.0081 28.5001C3' +
      '4.4413 28.537 34.7838 28.5613 34.986 28.5695C35.6333 28.5956 36.1647 28.27' +
      '25 36.5173 27.9287C36.8791 27.5759 37.1538 27.11 37.2981 26.618C37.442 26.' +
      '1275 37.4776 25.5321 37.2463 24.9617C37.0023 24.3598 36.5083 23.9048 35.82' +
      '65 23.6672C34.9042 23.3457 33.2807 22.9122 31.9254 22.5668C31.2397 22.392 ' +
      '30.6095 22.2366 30.1508 22.1248C30.0377 22.0973 29.935 22.0724 29.8444 22.' +
      '0505C29.7257 21.8284 29.5621 21.5248 29.3643 21.1652C28.94 20.3938 28.3561' +
      ' 19.3597 27.7205 18.3154C27.0883 17.2765 26.3907 16.2039 25.7397 15.3672C2' +
      '5.4157 14.9508 25.0832 14.5667 24.7591 14.2697C24.4751 14.0096 24.0444 13.' +
      '6688 23.5265 13.5945ZM28.8939 31.5143L30.0654 31.4304L30.3772 31.9781L30.5' +
      '03 32.1984C30.5841 32.3402 30.7013 32.545 30.8457 32.7963C31.1344 33.299 3' +
      '1.5315 33.9873 31.9647 34.7303C32.8372 36.2267 33.8375 37.9118 34.4021 38.' +
      '7645C34.6351 39.1164 35.0554 39.2116 35.4993 38.9865C35.7131 38.8781 35.85' +
      '73 38.7216 35.9242 38.5728C35.9814 38.4457 36.0057 38.2789 35.9034 38.0431' +
      'C35.4755 37.0566 34.47 34.6981 33.5734 32.5905C33.1246 31.5357 32.7026 30.' +
      '5424 32.3927 29.8128L31.8778 28.6L31.8763 28.598C31.8724 28.593 31.8668 28' +
      '.5864 31.8591 28.5791L31.8583 28.5784C31.844 28.5647 31.7571 28.4812 31.48' +
      '22 28.4812C31.1197 28.4812 30.5648 28.4052 30.0105 28.3149C29.4299 28.2203' +
      ' 28.7656 28.0951 28.1426 27.9715C28.0054 27.9442 27.8699 27.917 27.7374 27' +
      '.8901C28.3515 28.9824 28.7553 30.2088 28.8939 31.5143ZM24.4924 24.4897L24.' +
      '1299 22.9701L26.0547 22.4442L26.0582 22.4431L26.0585 22.4441L26.0602 22.44' +
      '96L26.0677 22.4735C26.0745 22.4951 26.0849 22.5278 26.0985 22.5699C26.1258' +
      ' 22.6544 26.1661 22.7767 26.2172 22.9255C26.3196 23.2243 26.4634 23.6241 2' +
      '6.6306 24.0346C26.7996 24.4498 26.9831 24.8527 27.1624 25.1679C27.252 25.3' +
      '255 27.3303 25.4427 27.3935 25.522C27.4134 25.5469 27.4287 25.5642 27.4395' +
      ' 25.5756C27.4751 25.5879 27.5585 25.6156 27.7104 25.6534C27.9281 25.7076 2' +
      '8.2151 25.7659 28.559 25.8262C29.2449 25.9465 30.1066 26.0662 30.9893 26.1' +
      '739C32.7235 26.3857 34.4661 26.5429 35.0288 26.5695C35.0452 26.5606 35.076' +
      '4 26.5403 35.1209 26.4969C35.226 26.3945 35.327 26.2321 35.379 26.055C35.4' +
      '314 25.8764 35.4126 25.7617 35.3929 25.7131L35.3923 25.7117C35.3845 25.692' +
      '2 35.3562 25.6213 35.1682 25.5558C34.3408 25.2674 32.8015 24.854 31.4315 2' +
      '4.5048C30.7545 24.3323 30.1313 24.1786 29.6774 24.068C29.4505 24.0127 29.2' +
      '66 23.9683 29.1385 23.9377L28.9917 23.9026L28.4959 23.785L28.2846 23.3791L' +
      '28.2735 23.3578L28.239 23.2921C28.2085 23.2341 28.1633 23.1485 28.1049 23.' +
      '0391C27.9883 22.8202 27.8194 22.5065 27.6118 22.129C27.196 21.373 26.6273 ' +
      '20.3661 26.0121 19.3552C25.3936 18.3389 24.7424 17.3423 24.1612 16.5954C23' +
      '.8691 16.22 23.6147 15.9338 23.4079 15.7443C23.2843 15.631 23.2115 15.584 ' +
      '23.1853 15.5676C22.8056 15.5337 22.3615 15.6963 21.8916 16.1078C21.4021 16' +
      '.5365 20.9974 17.1451 20.7758 17.7067C20.6778 17.9553 20.4913 18.7016 20.2' +
      '722 19.8115C20.0865 20.7521 19.8899 21.8873 19.7168 23.0557C21.4681 23.099' +
      '8 23.1014 23.6192 24.4924 24.4897ZM19.4507 25.0526L19.4737 25.0526C23.3074' +
      ' 25.0526 26.4668 27.9391 26.8974 31.6573L23.7382 31.8836L23.7188 31.886L23' +
      '.7176 31.8861L23.7165 31.8862L23.6968 31.8881C23.6762 31.8899 23.6417 31.8' +
      '927 23.5949 31.8953C23.5012 31.9006 23.3597 31.9053 23.1836 31.9012C22.828' +
      '2 31.893 22.3491 31.8495 21.8438 31.7134C20.8463 31.4447 19.8084 30.8424 1' +
      '9.3061 29.4465C19.2786 29.37 19.2357 29.1544 19.2187 28.7469C19.2026 28.36' +
      '34 19.2116 27.8842 19.2433 27.3312C19.2826 26.6437 19.3556 25.8656 19.4507' +
      ' 25.0526ZM17.4056 25.3424C17.3351 26.0005 17.2799 26.6351 17.2465 27.2169C' +
      '17.2128 27.8064 17.2006 28.3569 17.2204 28.8304C17.2392 29.2798 17.2891 29' +
      '.7483 17.4242 30.1236C18.2141 32.3189 19.9101 33.2638 21.3236 33.6445C22.0' +
      '237 33.8331 22.6694 33.8898 23.1373 33.9007C23.3729 33.9061 23.5677 33.9 2' +
      '3.7074 33.8922C23.7773 33.8882 23.8338 33.8838 23.8749 33.8801C23.8942 33.' +
      '8784 23.9102 33.8768 23.9226 33.8755L26.8612 33.665C26.3128 37.2522 23.214' +
      '1 40 19.4737 40C15.3461 40 12 36.6539 12 32.5263C12 29.1161 14.284 26.2394' +
      ' 17.4056 25.3424Z" fill="currentColor"/>',
    cane:
      '<path fill-rule="evenodd" clip-rule="evenodd" d="M24.5 15C24.5 16.3807 25.' +
      '6193 17.5 27 17.5C28.3807 17.5 29.5 16.3807 29.5 15V12C29.5 9.85055 28.829' +
      ' 7.9935 27.587 6.64805C26.3545 5.31278 24.6839 4.625 23 4.625C21.3161 4.62' +
      '5 19.6455 5.31278 18.413 6.64805C17.171 7.9935 16.5 9.85055 16.5 12C16.5 1' +
      '3.3807 17.6193 14.5 19 14.5C20.3807 14.5 21.5 13.3807 21.5 12C21.5 10.8994' +
      ' 21.829 10.319 22.087 10.0395C22.3545 9.74972 22.6839 9.625 23 9.625C23.31' +
      '61 9.625 23.6455 9.74972 23.913 10.0395C24.171 10.319 24.5 10.8994 24.5 12' +
      'V15ZM26.5 15C26.5 15.2761 26.7239 15.5 27 15.5C27.2761 15.5 27.5 15.2761 2' +
      '7.5 15V12C27.5 10.2701 26.9658 8.9237 26.1174 8.00461C25.2709 7.08756 24.1' +
      '368 6.625 23 6.625C21.8632 6.625 20.7291 7.08756 19.8826 8.00461C19.0342 8' +
      '.9237 18.5 10.2701 18.5 12C18.5 12.2761 18.7239 12.5 19 12.5C19.2761 12.5 ' +
      '19.5 12.2761 19.5 12C19.5 10.4799 19.9658 9.3888 20.6174 8.68289C21.2709 7' +
      '.97494 22.1368 7.625 23 7.625C23.8632 7.625 24.7291 7.97494 25.3826 8.6828' +
      '9C26.0342 9.3888 26.5 10.4799 26.5 12V15Z" fill="currentColor"/><path d="M' +
      '25.8607 38.8356C25.9411 38.3534 26.3583 38 26.8471 38H27.1529C27.6417 38 2' +
      '8.0589 38.3534 28.1393 38.8356L28.8059 42.8356C28.9075 43.4451 28.4375 44 ' +
      '27.8195 44H26.1805C25.5625 44 25.0925 43.4451 25.1941 42.8356L25.8607 38.8' +
      '356Z" fill="currentColor"/><path fill-rule="evenodd" clip-rule="evenodd" d' +
      '="M27 16.3664C26.6158 16.3664 26.2654 16.2279 26 16V37C26.2654 36.7721 26.' +
      '6158 36.6336 27 36.6336C27.3842 36.6336 27.7346 36.7721 28 37V16C27.7346 1' +
      '6.2279 27.3842 16.3664 27 16.3664Z" fill="currentColor"/><path fill-rule="' +
      'evenodd" clip-rule="evenodd" d="M24 43C24 42.4477 24.4477 42 25 42H29C29.5' +
      '523 42 30 42.4477 30 43C30 43.5523 29.5523 44 29 44H25C24.4477 44 24 43.55' +
      '23 24 43Z" fill="currentColor"/>'
  };
  /* `other`（型の定まらない用具）は車椅子の絵に落とす。専用の絵が無い
     ものに抽象記号（箱＋丸）を当てると「何の絵か言えない」ものが並ぶので、
     いちばん近い実物の絵を借り、区別は用具名の文字で付ける。 */
  EQUIP_IC.other = EQUIP_IC.chair;
  /* 用具のグリフ。塗りのシルエットなので stroke は指定しない
     （パスが fill="currentColor" を持つので、色は CSS の color で決まる）。 */
